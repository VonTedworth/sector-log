...
**Current build: 16** (shown top of the page as `build N · date`). Bump the `BUILD` constant in `index.html` on every commit so the running version is verifiable on the device.
...
- **Total time comes from the rotor times, not HCM's Total column.** Ed logs rotors-running, so whenever both rotor times exist Total is derived from **In − Out** at build (past-midnight preserved by the monotonic Out ≤ Off ≤ On ≤ In ordering). HCM's `Total Time` column is unreliable — on several sectors it holds airborne time (On − Off) and on some it is simply wrong — and is not trusted. Where the derived total disagrees with HCM's column by more than a minute the sector shows an informational `HCM TOT h:mm` chip (HCM's claimed value) so you can see which ones HCM got wrong; it does not block Send. Sectors missing a rotor time fall back to HCM's total, which stays editable.
- **Rotor-time correction**: some pilots enter identical rotor and airborne times (Out = Off, On = In). Ed logs rotors-running time, so when they're equal the app subtracts 5 min from the start and adds 1 min to the stop and recomputes total. Flagged `ROTOR +6` (the `HCM TOT` chip is suppressed here — the difference is explained by this correction).
...
- **Known sites** are held in a reference list (`KNOWN_SITES`) carrying each site's LogTen identifier, name variants (for recognition), and two flags: `rooftop` (elevated helipad → drives Platform T/O/Ldg) and `scene` (the generic `KSS HEMS SITE`). No coordinates are stored. Seeded rooftop hospitals: Kings College, St George's, Queen Alexandra, Frimley Park, and the Brighton main. **William Harvey is a ground site (rooftop off)** — a company HLS-directory landing site, not an elevated helipad, so it gets no platform counts. Airfields (EGKR/EGTO/EGKA/EGKB/EGMD) are ground too. Brighton has two distinct entries: **SE02 Brighton Royal Sussex Hospital** (rooftop, main) and **Royal Sussex Hospital Secondary** / East Brighton Park / "SE02A Royal Sussex - Alternate" (ground-level, no platform landing) — names containing "Alternate", "Secondary/Secondry" or "East Brighton Park" always resolve to the Secondary. A resolved place always reports the canonical `KNOWN_SITES` id (so its flags apply and that id is what's sent to LogTen) even when an alias target spells it differently.
- **Recognition** is case-, punctuation- and operational-suffix tolerant, and tolerant of one trailing city/qualifier word: `Bagshot` and `Bagshot HOS` match the same scene (trailing HOS/RTC/OOH/HLS stripped); `St Georges Hospital London` matches the id `ST GEORGE'S HOSPITAL` (apostrophe and full stop ignored, trailing `London` allowed). It stays otherwise strict — at most one word of difference, so `Ashford` never matches `Ashfield` and `A B London` never matches `A B Ashford`. Every seeded alias target resolves to a known site, so no site's flags are silently inert.
